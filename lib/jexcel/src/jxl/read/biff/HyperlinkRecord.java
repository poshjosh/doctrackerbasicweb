...
public class HyperlinkRecord extends RecordData implements Hyperlink
{
   ...
   * The logger
   */
  private static Logger logger = Logger.getLogger(HyperlinkRecord.class.getName());
  ...
  HyperlinkRecord(Record t, Sheet s, WorkbookSettings ws)
  {
    ...
    if (linkType == urlLink)
    {
      ...
      {
        logger.warning("URL " + urlString + " is malformed.  Trying a file");
        try
        {
          linkType = fileLink;
        ...
        {
          logger.warning("Cannot set to file.  Setting a default URL");
    ...
    else if (linkType == fileLink)
    {
      ...
      {
        logger.warning("Exception when parsing file " + 
                    e.getClass().getName() + ".");
        file = new File(".");
      }
    }
    ...
    {
      // give up
      logger.warning("Cannot determine link type");
      return;
    }
  }
