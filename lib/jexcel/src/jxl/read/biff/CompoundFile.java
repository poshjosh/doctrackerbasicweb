...
public final class CompoundFile extends BaseCompoundFile
{
  ...
  private void readSmallBlockDepot() throws BiffException
  {
    ...
    if (sbdBlock == -1)
    {
      logger.warning("invalid small block depot number");
      return;
    }
    ...
    while (offset < rootEntry.length)
    {
      ...
      if (ps.name == null || ps.name.length() == 0)
      {
        if (ps.type == ROOT_ENTRY_PS_TYPE)
        {
          ...
          logger.warning("Property storage name for " + ps.type + 
        ...
        else
        {
          if (ps.size != 0)
          {
            logger.warning("Property storage type " + ps.type + 
    ...
    throws BiffException
  {
    ...
    if (multiple)
    {
      logger.warning("found multiple copies of property set " + name);
    }
  ...
  private byte[] getBigBlockStream(PropertyStorage ps)
  {
    ...
    {
      logger.warning("Property storage size inconsistent with block chain.");
    }
    ...
    throws BiffException
  {
    ...
    for (; blockCount <= smallBlockChain.length && block != -2; blockCount++)
    {
      ...
      if (block == -1)
      {
        logger.warning("Incorrect terminator for small block stream " + ps.name);
