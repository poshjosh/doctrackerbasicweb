...
final class SheetReader
{
   ...
   * The logger
   */
  private static Logger logger = Logger.getLogger(SheetReader.class.getName());
  ...
  private void addCell(Cell cell)
  {
    ...
    if (cell.getRow() < numRows && cell.getColumn() < numCols)
    {
      if (cells[cell.getRow()][cell.getColumn()] != null)
      {
        ...
        logger.warning("Cell " + sb.toString() +
  ...
  final void read()
  {
    ...
    while (cont)
    {
      ...
      {
        logger.warning("Biff code zero found");

        // Try a dimension record
        if (r.getLength() == 0xa)
        {
          logger.warning("Biff code zero found - trying a dimension record.");
          r.setType(Type.DIMENSION);
        }
        else
        {
          logger.warning("Biff code zero found - Ignoring.");
        }
      }
      ...
      else if (type == Type.NOTE)
      {
        if (!workbookSettings.getDrawingsDisabled())
        {
          ...
          if (comment == null)
          {
            logger.warning(" cannot find comment for note id " +
      ...
      else if (type == Type.SHAREDFORMULA)
      {
        ...
        {
          logger.warning("Shared template formula is null - " +
      ...
      else if (type == Type.DVAL)
      {
        if (!workbookSettings.getCellValidationDisabled())
        {
          ...
          {
            logger.warning("object id " + dvlr.getObjectId() + " referenced " +
      ...
      else if (type == Type.DV)
      {
        if (!workbookSettings.getCellValidationDisabled())
        {
          ...
          {
            logger.warning("cannot add data validity settings");
          }
        }
      }
      else if (type == Type.OBJ)
      {
        ...
        if (!workbookSettings.getDrawingsDisabled())
        {
          ...
          {
            logger.warning("Cannot find drawing record - using continue record");
      ...
      else if (type == Type.BOF)
      {
        ...
        if (br.isChart())
        {
          ...
          {
            logger.warning("only biff8 charts are supported");
          }
          else
          {
            if (drawingData == null)
    ...
    if (!comments.isEmpty())
    {
      logger.warning("Not all comments have a corresponding Note record");
    }
  }
                              ...
                              double height)
  {
    ...
    if (c == null)
    {
      logger.warning("Cell at " + CellReferenceHelper.getCellReference(col, row) +
    ...
    {
      logger.warning("Not able to add comment to cell type " +
                  c.getClass().getName() +
                                  ...
                                  HashMap comments)
  {
    if (msoRecord == null)
    {
      logger.warning("Object record is not associated with a drawing " +
                  " record - ignoring");
      return;
    }
    ...
    try
    {
      ...
      {
        logger.warning(objRecord.getType() + " Object on sheet \"" +
                    sheet.getName() +
