...
public class ExternalSheetRecord extends RecordData
{
   ...
   * The logger
   */
  private static Logger logger = Logger.getLogger(ExternalSheetRecord.class.getName());
  ...
  ExternalSheetRecord(Record t, WorkbookSettings ws)
  {
    ...
    {
      xtiArray = new XTI[0];
      logger.warning("Could not process external sheets.  Formulas may " +
                  "be compromised.");
      return;
    }
  ...
  ExternalSheetRecord(Record t, WorkbookSettings settings, Biff7 dummy)
  {
    ...
    logger.warning("External sheet record for Biff 7 not supported");
  }
