...
class ReadFormulaRecord extends CellValue implements FormulaData
{
   ...
   * The logger
   */
  private static Logger logger = Logger.getLogger(ReadFormulaRecord.class.getName());
  ...
  void columnInserted(Sheet s, int sheetIndex, int col)
  {
    ...
    {
      logger.warning("cannot insert column within formula:  " + e.getMessage());
    }
  }
  ...
  void columnRemoved(Sheet s, int sheetIndex, int col)
  {
    ...
    {
      logger.warning("cannot remove column within formula:  " + e.getMessage());
    }
  }
  ...
  void rowInserted(Sheet s, int sheetIndex, int row)
  {
    ...
    {
      logger.warning("cannot insert row within formula:  " + e.getMessage());
    }
  }
  ...
  void rowRemoved(Sheet s, int sheetIndex, int row)
  {
    ...
    {
      logger.warning("cannot remove row within formula:  " + e.getMessage());
    }
  }
                                              ...
                                              WorkbookSettings ws)
  {
    ...
    {
      logger.warning("cannot import formula:  " + e.getMessage());
      return false;
    }
  }
}
