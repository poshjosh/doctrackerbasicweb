...
class SheetCopier
{
  private static Logger logger = Logger.getLogger(SheetCopier.class.getName());
  ...
  public void copySheet()
  {
    ...
    if (fromSheet.getPLS() != null)
    {
      ...
      {
        logger.warning("Cannot copy Biff7 print settings record - ignoring");
      }
      else
      {
  ...
  public void importSheet()
  {
    ...
    {
      logger.warning("Importing of charts is not supported");
      /*
      sheetWriter.setCharts(fromSheet.getCharts());
  ...
  private WritableCell deepCopyCell(Cell cell)
  {
    ...
    if (c instanceof ReadFormulaRecord)
    {
      ...
      if (crossSheetReference)
      {
        try
        {
        logger.warning("Formula " + rfr.getFormula() +
                    " in cell " + 
        ...
        {
          logger.warning("Formula  in cell " + 
                      CellReferenceHelper.getCellReference(cell.getColumn(),
                                                           cell.getRow()) +
    ...
    for (int i = 0 ; i < nameRecords.length ;i++)
    {
      ...
      for (int j = 0; j < nameRanges.length; j++)
      {
        ...
        if (fromSheetIndex == nameSheetIndex)
        {
          ...
          {
            logger.warning("Named range " + name + 
