...
class WritableSheetImpl implements WritableSheet
{
   ...
   * The logger
   */
  private static Logger logger = Logger.getLogger(WritableSheetImpl.class.getName());
    ...
    throws WriteException, RowsExceededException
  {
        ...
        curSharedValidation)
    {
      ...
      logger.warning("Cannot add cell at " + 
  ...
  public void setColumnView(int col, CellView view)
  {
    ...
    {
      logger.warning("Maximum number of format records exceeded.  Using " +
                  "default format.");
    ...
    throws WriteException, RowsExceededException
  {
    ...
    {
      logger.warning("Cannot merge cells - top left and bottom right "+
    ...
    throws WriteException, RowsExceededException 
  {
    if (row2 < row1)
    {
      logger.warning("Cannot merge cells - top and bottom rows incorrectly " + 
                  "specified");
    }
  ...
  private String validateName(String n)
  {
    ...
    {
      logger.warning("Sheet name " + n + " too long - truncating");
      n = n.substring(0, maxSheetNameLength);
    }

    if (n.charAt(0) == '\'')
    {
      logger.warning("Sheet naming cannot start with \' - removing");
      n = n.substring(1);
    }
    ...
    for (int i = 0 ; i < illegalSheetNameCharacters.length ; i++)
    {
      ...
      if (n != newname)
      {
        logger.warning(illegalSheetNameCharacters[i] + 
  ...
  void removeDataValidation(CellValue cv)
  {
    ...
    if (validatedCells != null)
    {
      ...
      if (!result)
      {
        logger.warning("Could not remove validated cell " + 
                    CellReferenceHelper.getCellReference(cv));
      }
    }
  }
    ...
    throws WriteException
  {
    ...
    {
      logger.warning("Cannot extend data validation for " +
    ...
    for (int y = startRow; y <= endRow; y++)
    {
      if (rows[y] != null)
      {
        ...
        for (int x = startColumn; x <= endCol; x++)
        {
          ...
          {
            logger.warning("Cannot apply data validation from " +
 ...
 /**
    ...
    throws WriteException
  {
    ...
    if (dvp.extendedCellsValidation())
    {
      ...
      {
        logger.warning("Cannot remove data validation from " +
                    CellReferenceHelper.getCellReference(dvp.getFirstColumn(),
                                                         dvp.getFirstRow()) +
                    "-" +
                    CellReferenceHelper.getCellReference(dvp.getLastColumn(),
