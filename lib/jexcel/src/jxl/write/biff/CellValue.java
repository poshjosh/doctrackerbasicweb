  ...
  implements WritableCell
{
   ...
   * The logger
   */
  private static Logger logger = Logger.getLogger(CellValue.class.getName());
  ...
  void decrementRow()
  {
    ...
    if (features != null)
    {
      ...
      {
        logger.warning("need to change value for drop down drawing");
      }
    }
  }
  ...
  public void setCellFeatures(WritableCellFeatures cf)
  {
    if (features != null) 
    {
      logger.warning("current cell features for " + 
          ...
          features.getDVParser().extendedCellsValidation())
      {
        ...
        logger.warning("Cannot add cell features to " + 
                    CellReferenceHelper.getCellReference(this) + 
