...
public class DVParser
{
   ...
   * The logger
   */
  private static Logger logger = Logger.getLogger(DVParser.class.getName());
                  ...
                  WorkbookSettings ws)
  {
    ...
    {
      logger.warning(e.getMessage() + " for cells " + 
  ...
  public DVParser(Collection strings)
  {
    ...
    if (strings.size() == 0)
    {
      logger.warning("no validation strings - ignoring");
    }
    ...
    {
      logger.warning("Validation list exceeds maximum number of characters - " +
                  "truncating");
  ...
  public DVParser(DVParser copy)
  {
    ...
    else
    {
      ...
      {
        logger.warning("Cannot parse validation formula:  " + e.getMessage());
      }
    }
