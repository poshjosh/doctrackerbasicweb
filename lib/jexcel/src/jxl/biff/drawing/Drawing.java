...
public class Drawing implements DrawingGroupObject, Image
{
   ...
   * The logger
   */
  private static Logger logger = Logger.getLogger(Drawing.class.getName());
  ...
  private void initialize()
  {
    ...
    {
      logger.warning("Unknown shape type");
    }
    ...
    else
    {
      ...
      {
        logger.warning("no filename property for drawing");
    ...
    {
      logger.warning("client anchor not found");
    }
    else
    {
    ...
    if (blipId == 0)
    {
      logger.warning("linked drawings are not supported");
    }
  ...
  private double getWidthInPoints()
  {
    if (sheet == null)
    {
      logger.warning("calculating image width:  sheet is null");
      return 0;
    }
  ...
  private double getHeightInPoints()
  {
    if (sheet == null)
    {
      logger.warning("calculating image height:  sheet is null");
      return 0;
    }
